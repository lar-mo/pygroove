...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--album-id", type=int, help="Specific album ID to update")
        parser.add_argument("--discogs-id", type=int, help="Specific Discogs release ID to use (requires --album-id)")
        parser.add_argument("--all", action="store_true", help="Update all albums in database")
        parser.add_argument(
            "--missing-only", action="store_true", help="Only update albums missing cover images or tracks"
        )

    def handle(self, *args, **options):
        # Initialize Discogs client
        d = discogs_client.Client("PyGroove/1.0", user_token=settings.DISCOGS_TOKEN)
        ...
        try:
            ...
            for track_data in tracklist:
                ...
                if track_number > 0:  # Only add tracks with valid numbers
                    Track.objects.create(album=album, title=title, track_number=track_number, duration=duration)
